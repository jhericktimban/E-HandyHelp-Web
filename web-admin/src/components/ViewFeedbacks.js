        ...
        <div className="view-feedbacks-container">
            ...
            <div className="feedbacks-list">
                {feedbacks.map((feedback, index) => {
                    const title = feedback.sent_by === 'user' ? 'Feedback for User' : 'Feedback for Handyman';
                    const feedbackByName = feedback.sent_by === 'user'
                        ? `${feedback.userId?.fname || 'N/A'} ${feedback.userId?.lname || 'N/A'}`
                        : `${feedback.handymanId?.fname || 'N/A'} ${feedback.handymanId?.lname || 'N/A'}`;
                        ...
                        <Card key={index} className="feedback-card">
                            <Card.Body>
                                ...
                                <Card.Text>
                                    ...
                                    <strong>{feedback.sent_by === 'user' ? 'Handyman' : 'User'}:</strong> 
            ...
            {selectedFeedback && (
                <Modal show={showModal} onHide={handleCloseModal}>
                    <Modal.Header closeButton>
                        <Modal.Title>{selectedFeedback.sent_by === 'user' ? 'Feedback for User' : 'Feedback for Handyman'}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <p><strong>Feedback Text:</strong> {selectedFeedback.feedbackText}</p>
                        <p><strong>Feedback By:</strong> {selectedFeedback.sent_by === 'user' 
                            ? `${selectedFeedback.userId?.fname || 'N/A'} ${selectedFeedback.userId?.lname || 'N/A'}` 
                            : `${selectedFeedback.handymanId?.fname || 'N/A'} ${selectedFeedback.handymanId?.lname || 'N/A'}`}</p>
                        <p><strong>{selectedFeedback.sent_by === 'user' ? 'Handyman' : 'User'}:</strong> 
                        {selectedFeedback.sent_by === 'user' 
                            ? `${selectedFeedback.handymanId?.fname || 'N/A'} ${selectedFeedback.handymanId?.lname || 'N/A'}` 
                            : `${selectedFeedback.userId?.fname || 'N/A'} ${selectedFeedback.userId?.lname || 'N/A'}`}</p>
